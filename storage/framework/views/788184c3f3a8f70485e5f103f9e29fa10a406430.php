        ...
        <script type="text/javascript">
            function change_lang(lang){
              $.ajax(
				{
					type: "GET",
					url: "<?php echo e(route('change_lang',$subdomain)); ?>",
					data:{lang:lang}, 
		...
		//	var to= $("#outdoor input[name='date_to']").val();*/
			...
			$.ajax({
                ...
                beforeSend: function() { $("#outdoor_data #load").show(); },
                        ...
                        history.pushState('', '',"<?php echo e(url('admin/outdoor')); ?>"+"?employee_name="+employee_name+"&customer_id="+customer_id +"&status="+status+"&date="+date+"&created_by="+created_by+"&branch="+branch+"&visit_type="+visit_type+"&to="+to+"&department"+department);
...
*/
 	...
 	/* search visit visit_report_details*/
    	...
    	$("#search_visit_report").click(function(e){
		    ...
		    $('#visit_printlink').attr("href","<?php echo e(url('admin/visitPrint/visit')); ?>"+"?user_id="+user_id+"&customer_id="+customer_id+"&visit_type="+visit_type+"&to="+to+"&from="+from+"&department="+department+"&branch="+branch+"&status="+status+"&created_by="+created_by+"&is_registered="+is_registered);
		    var visit_type= $("#visit_report .visit_types").val();
			...
			//	headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                ...
                beforeSend: function() { $(".visit_report_data #load").show(); },
				     ...
				     history.pushState('', '',"<?php echo e(url('admin/visitReport')); ?>"+"?user_id="+user_id+"&from="+from+"&to="+to+"&visit_type="+visit_type+"&department="+department+"&branch="+branch+"&customer_id="+customer_id+"&status="+status+"&created_by="+created_by+"&is_registered="+is_registered);
        ...
        $("#search_daily").click(function(){
       	...
       	$("#search_userReport").click(function(){
            ...
            if(user==null){
                ...
                $(".alert_msg").text("");
		    $('#userReport_printlink').attr("href","<?php echo e(url('admin/userReportPrint/userReport?id="+user+"')); ?>"+"?date_to="+date_to+"&date_from="+date_from+"&id="+user);
                        ...
                        const params = new URLSearchParams(window.location.search);
                          ...
                          if (typeof outdoor_ids !== 'undefined'){
                              history.pushState('', '',url11+"?page=" + page);
    				            ...
    				            history.pushState('', '',"<?php echo e(url('admin/visitReport')); ?>"+"?user_id="+user_id+"&customer_id="+customer_id+"&visit_type="+visit_type+"&to="+to+"&from="+from+"&department="+department+"&branch="+branch+"&status="+status+"&created_by="+created_by+"&is_registered="+is_registered+"&page="+page);
    				    }
...
$("#edit_evaluationemp").on('show.bs.modal', function(event) 
	...
	<?php /**PATH /home/dawam/public_html/manger/resources/views/layout/partials/footer-scripts.blade.php ENDPATH**/ ?>