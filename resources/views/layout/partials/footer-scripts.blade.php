...
<script src="{{asset('js/jquery-3.2.1.min.js')}}"></script>
        ...
        <script type="text/javascript">
            function change_lang(lang){
              $.ajax(
				{
					type: "GET",
					url: "{{route('change_lang',$subdomain)}}",
					data:{lang:lang}, 
		...
		<script>
		 ...
		 $(document).ready(function(){
             ...
             baseUrl=baseUrl1+"/";
             img_url=getUrl .protocol + "//" + getUrl.host + "/";
			...
			id= $(this).attr('id');
			 ...
			 let _token= $('meta[name="csrf-token"]').attr('content');
				...
				{
					type: "POST",
					url: "{{route('upgrade-subscribe',$subdomain)}}",
	...
	$("#edit_evaluationjob").on('show.bs.modal', function(event) 
    {
				...
				$.ajax({
                   ...
                   // $("#edit_evaluationjob form input[name='target_location1']").prop("checked",data.jobs.target_location_check?true:false);
					if(data.jobs.target_location_check==1?$("#edit_evaluationjob form .target_location1").prop("checked",true):$("#edit_evaluationjob form .target_location").prop("checked",true));
			  	    ...
			  	    if(data.jobs.outdoor_without_attend==1?$("#edit_evaluationjob form .outdoor_without_attend1").prop("checked",true):$("#edit_evaluationjob form .outdoor_without_attend").prop("checked",true));
		...
		$("#edit_employee button,#add_employee button").click(function(e){
		    ...
		    if($(this).attr('type')=="add"){
				...
				data11={name:$( "#add_employee form input[name*='name']" ).val(),
			...
			if($(this).attr('type')=="edit"){
               url=$('#edit_employee form').attr('action'),
                data11={name:$( "#edit_employee form input[name*='name']" ).val(),
                    ...
                    branch:branch,//$( "#edit_employee select[name='branch']" ).val()
		...
		$.ajax({
				url:"{{route('status-shift',$subdomain)}}",    
				...
				$.ajax({
					...
					$.each(data, function( index,shift){
                        ...
                        //console.log(shift.shift_default);
                         ...
                         $("#edit_shift form select[name='shift_default']").val(shift.shift_default).change();
 ...
 */
     ...
     function add_client_sch(selector){
          // alert("4");
		    ...
		    ++i_client;
	     	...
	     	$("#"+selector+" .client_sch").last().after('<div class="client_sch appoint_'+i_client+' row" id="'+i_client+'" ><div class="col-sm-4"> <label class="col-form-label">{{__("trans.day")}} <span class="text-danger" >*</span></label><select class="select day_'+i_client+'" name="day[]"><option value="6">{{__("trans.saturday")}}</option><option value="0">{{__("trans.sunday")}}</option><option value="1">{{__("trans.monday")}}</option><option value="2">{{__("trans.thursday")}}</option><option value="3">{{__("trans.wednsday")}}</option><option value="4">{{__("trans.tuesday")}}</option><option value="5">{{__("trans.friday")}}</option></select></div><div class="col-sm-3"><label>{{__("trans.From")}} <span class="text-danger">*</span></label><div class="input-group time timepicker"><input type="time" class="form-control" value="00:00" name="start_time[]"/></div></div><div class="col-sm-3"><label>{{__("trans.To")}}<span class="text-danger">*</span></label><div class="input-group time timepicker"><input type="time" value="00:00" class="form-control" name="end_time[]"/></div></div><div class="col-sm-2" onclick="delete_appoint('+i_client+');"><i class="fa fa-minus"></i></div></div>');
     }
			...
			$.ajax({
				...
				$.each(data, function( index,employee){
					...
					$( "select[name='job_id']" ).val(employee.job_id);
					  ...
					  department= (employee.department_id==null)?"all":employee.department_id;
				    	$( "#edit_employee select[name='department']" ).val(department);
                      	$( "#edit_employee select[name='department']" ).val(department).change();
                    	
                       branch= (employee.branch_id==null)?"all":employee.branch_id;
                        $( "#edit_employee select[name='branch']" ).val(branch);
                       $( "#edit_employee select[name='branch']").val(branch).change();
                   
                     console.log($("#edit_employee select[name='branch']").val());
	...
	/*fix_holidays delete*/
		...
		/*	$('#from_time').timepicker({
			...
			$.ajax({
					url:"{{route('status-client',$subdomain)}}",    
                       ...
                       $("#attendance_info .user_name_view").empty();
						...
						$.each(data.attendance, function(index,attend){
                            ...
                            if(attend.details_type=="client"){
                                console.log("client");
                                $("#attendance_info .active_in").append("<h3 class='detail_header'>"+"{{__('trans.client_attendance')}}"+"</h3>");
                                ...
                                $("#attendance_info .active_in").append("<h3 class='detail_header'>"+"{{__('trans.branch_attendance')}}"+"</h3>");
                                $.each(attend.details, function(index,detail){
                                 ...
                                 $("#attendance_info .active_in").append("<h3 class='detail_header'>"+"{{__('trans.none detected')}}"+"</h3>");
	...
	$(".leave_type").change(function() {
		...
		$.ajax({
				    url:"{{route('get_available',$subdomain)}}",    
...
/*  leave edit */
	$("#edit_leave").on('show.bs.modal', function(event) {
        ...
        /* leave setting */
            ...
            $("#switch_annual,#switch_sick,#switch_maternity,#switch_paternity,#switch_hours,#switch_hospitalisation,#switch_custom01").on('change', function() {
		...
		$("#search_attendance").click(function(){
			...
			$.ajax({
                ...
                beforeSend: function() { $("#attendance_data #load").show(); },
                    ...
                    history.pushState('', '',"{{url('admin/attendance')}}"+"?employee_name="+employee_name+"&month="+month +"&year="+year);
		...
		$("#search_leave").click(function(){
			...
			$.ajax({
                ...
                beforeSend: function() { $("#leave_data #load").show(); },
                        ...
                        history.pushState('', '',"{{url('admin/leaves')}}"+"?employee_name="+employee_name+"&status="+status+"&from="+from +"&leave_type="+leave_type+"&to="+to+"&department="+department+"&branch="+branch);
		...
		$("#search_task").click(function(){
			...
			$.ajax({
                ...
                beforeSend: function() { $(".task_result #load").show(); },
                        ...
                        history.pushState('', '',"{{url('admin/task')}}"+"?employee_name="+employee_name+"&date_from="+date_from +"&status="+status+"&date_to="+date_to+"&department="+department+"&branch="+branch);
		...
		//	var to= $("#outdoor input[name='date_to']").val();*/
			...
			$.ajax({
                ...
                beforeSend: function() { $("#outdoor_data #load").show(); },
                        ...
                        history.pushState('', '',"{{url('admin/outdoor')}}"+"?employee_name="+employee_name+"&customer_id="+customer_id +"&status="+status+"&date="+date+"&created_by="+created_by+"&branch="+branch+"&visit_type="+visit_type+"&to="+to+"&department"+department);
...
*/
 	...
 	/* search visit visit_report_details*/
    	...
    	$("#search_visit_report").click(function(e){
		    ...
		    $('#visit_printlink').attr("href","{{url('admin/visitPrint/visit')}}"+"?user_id="+user_id+"&customer_id="+customer_id+"&visit_type="+visit_type+"&to="+to+"&from="+from+"&department="+department+"&branch="+branch+"&status="+status+"&created_by="+created_by+"&is_registered="+is_registered);
		    var visit_type= $("#visit_report .visit_types").val();
			...
			//	headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
				type:"get",
				url:getHref1,
				data:{user_id:user_id,from:from,to:to,visit_type:visit_type,department:department,branch:branch,customer_id:customer_id,status:status,created_by:created_by,is_registered:is_registered},
                beforeSend: function() { $(".visit_report_data #load").show(); },
				}).done(function(data) {
				     history.pushState('', '',"{{url('admin/visitReport')}}"+"?user_id="+user_id+"&from="+from+"&to="+to+"&visit_type="+visit_type+"&department="+department+"&branch="+branch+"&customer_id="+customer_id+"&status="+status+"&created_by="+created_by+"&is_registered="+is_registered);
...
/*job */
		...
		/*leave_status*/
			...
			$(".leave_status_continue-btn").click(function(){
				...
				$.ajax({
					url:"{{route('change-status',$subdomain)}}",    
                    ...
                    if(client.appointments!=""||client.appointments!=[]){
                        $.each(JSON.parse(client.appointments), function(index,appoint){
                            ...
                            if(appoint.day==7){
                               ...
                               $(".client_appoints").append('<div  class="client_sch appoint_'+i_client+' row" id="'+i_client+'" ><div class="col-sm-4"> <label class="col-form-label">{{__("trans.day")}} <span class="text-danger" >*</span></label><select class="select day_'+i_client+'" name="day[]" value="'+appoint.day+'"><option value="6">{{__("trans.saturday")}}</option><option value="0">{{__("trans.sunday")}}</option><option value="1">{{__("trans.monday")}}</option><option value="2">{{__("trans.thursday")}}</option><option value="3">{{__("trans.wednsday")}}</option><option value="4">{{__("trans.tuesday")}}</option><option value="5">{{__("trans.friday")}}</option></select></div><div class="col-sm-3"><label>{{__("trans.From")}} <span class="text-danger">*</span></label><div class="input-group time timepicker"><input type="time" class="form-control" name="start_time[]" value="'+appoint.start_time+'"/></div></div><div class="col-sm-3"><label>{{__("trans.To")}}<span class="text-danger">*</span></label><div class="input-group time timepicker"><input type="time" class="form-control" name="end_time[]" value="'+appoint.end_time+'"/></div></div><div class="col-sm-2" onclick="delete_appoint('+i_client+');"><i class="fa fa-minus"></i></div></div>');
	...
	/* delete client type */
		...
		$('.DialyReport').click(function(){
			$.ajax({
					url: "{{route('dialy-present',$subdomain)}}",    
        ...
        $("#search_daily").click(function(){
       	...
       	$("#search_userReport").click(function(){
            ...
            if(user==null){
                ...
                $(".alert_msg").text("");
		    $('#userReport_printlink').attr("href","{{url('admin/userReportPrint/userReport?id="+user+"')}}"+"?date_to="+date_to+"&date_from="+date_from+"&id="+user);
...
$("#search_available").click(function()
          ...
          //alert(type);
			...
			$.ajax({
				...
				}).done(function(data)
                {
                  //console.log(data);
                 // return;
                    history.pushState('', '',"{{url('admin/attendance/employee_available')}}"+"/"+type+"?employee_name="+employee_name+"&client="+client +"&branch="+branch);
   ...
   // company_img upload
    $(".but_upload").change(function(){
        ...
        if(files.length > 0 ){
           ...
           $.ajax({
              headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
              url: '{{route("images.store",$subdomain)}}',
              type: 'post',
              data:fd,  
              contentType: false,
              processData: false,
   ...
   $("#edit_company .delete_img").click(function(){
			...
			$.ajax({
					url:'{{route("company-img-delete",$subdomain)}}',    
					data:{id:del_img_id},
					 headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
					type:"post"
					}).done(function(data) {
	   ...
	   var interval =30000;  
		function doAjax() {
			$.ajax({
			        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
					type: 'POST',
					url: "{{route('get-notification',$subdomain)}}",
					success: function (data) {
					  ///  $('#ajax_notfy').empty(); 
    					$('.notfy').empty();  
    					$('.notfy').append(data);   
                    ...
                    beforeSend: function() { $(".visit_report_data #load").show(); },
                        ...
                        const params = new URLSearchParams(window.location.search);
                          ...
                          if (typeof outdoor_ids !== 'undefined'){
                              history.pushState('', '',url11+"?page=" + page);
                          }else{
           ...
           //   return;
			...
			$.ajax({
				...
				}).done(function(data)
                {
                  //console.log(data);
                 // return;
                    history.pushState('', '',"{{url('admin/evaluation_employes')}}"+"?employee_name="+employee_name+"&department="+department +"&branch="+branch+"&month="+month+"&year="+year);
